/**
 * dashboard.js - Ultimate Stable Object-Oriented Version
 * No more ReferenceErrors or Duplicate declarations.
 */
...
const DashboardApp = {
    ...
    async handleAuth() {
        ...
        try {
            ...
            if (data.authenticated) {
                ...
                this.renderUserBasics(data.user);
            } else {
    ...
    handleSocketMessage(data) {
        ...
        switch (data.type) {
            case 'connectResult': 
                ...
                if(data.channelInfo) this.renderStreamerStatus(data.channelInfo, data.liveStatus);
                break;
    ...
    renderStreamerStatus(info, live) {
    ...
    renderSongUI(state) {
        ...
        if (list) list.innerHTML = state.queue.length ? state.queue.map(s => `<div class="queue-item"><span>${this.utils.esc(s.title)}</span></div>`).join('') : '<div class="empty-state">대기열 비어있음</div>';
    ...
    deleteCommand(t) { if(confirm('삭제할까요?')) this.send({ type: 'removeCommand', data: { trigger: t } }); },
    ...
    deleteMacro(id) { if(confirm('삭제할까요?')) this.send({ type: 'removeMacro', data: { id } }); },
    ...
    deleteCounter(t) { if(confirm('삭제할까요?')) this.send({ type: 'removeCounter', data: { trigger: t } }); },
    ...
    savePointsSettings() {
        ...
        this.utils.notify('저장 완료', 'success');
    },
    ...
    resetGreetHistory() { if(confirm('기록을 초기화할까요?')) this.send({ type: 'resetGreetHistory' }); },
    toggleParticipation() { this.send({ type: 'toggleParticipation' }); },
    clearParticipants() { if(confirm('데이터를 초기화할까요?')) this.send({ type: 'clearParticipants' }); },
...
window.sendWebSocket = (data) => DashboardApp.send(data); // Legacy support for vote-system.js
