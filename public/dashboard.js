// chzzk-bot-v2/public/dashboard.js - Expert Finalized Version
...
function handleGlobalMessage(data) {
    switch (data.type) {
        ...
        case 'commandsUpdate': 
            window.dashboardData.commands = data.payload || [];
            renderList('commands-list', window.dashboardData.commands, 'deleteCommand', 'editCommand'); 
            break;
        case 'macrosUpdate': 
            window.dashboardData.macros = data.payload || [];
            renderList('macros-list', window.dashboardData.macros, 'deleteMacro', 'editMacro'); 
            break;
        case 'countersUpdate': 
            window.dashboardData.counters = data.payload || [];
            renderList('counters-list', window.dashboardData.counters, 'deleteCounter', 'editCounter'); 
            break;
...
function renderList(id, list, deleteFn, editFn) {
    ...
    el.innerHTML = list.length ? list.map(item => `
        <div class="item-card card">
            <div class="item-info">
                <b>${ui.escape(item.triggers ? item.triggers[0] : item.trigger)}</b>
                <span>${ui.escape(item.response || item.message)}</span>
            </div>
            <div class="item-actions">
                <button class="btn-icon" onclick="window.${editFn}('${item.id || item.trigger}')" title="수정"><i class="fas fa-edit"></i></button>
                <button class="btn-icon btn-danger" onclick="window.${deleteFn}('${item.id || item.trigger}')" title="삭제"><i class="fas fa-trash"></i></button>
            </div>
...
function updateSongUI(state) {
    ...
    if (list) list.innerHTML = state.queue.length ? state.queue.map(s => `<div class="queue-item"><span>${ui.escape(s.title)}</span></div>`).join('') : '<div class="empty-state">대기열이 비어있습니다.</div>';
...
function updateParticipationUI(state) {
    ...
    if (wait) wait.innerHTML = state.queue.map(p => `
        <div class="participant-item">
            <span>${ui.escape(p.nickname)}</span>
            <button class="btn-sm" onclick="sendWebSocket({type:'moveToParticipants',data:{userIdHash:'${p.userIdHash}'}})">승인</button>
        </div>`).join('');
    const active = document.getElementById('active-participants');
    if (active) active.innerHTML = state.participants.map(p => `<div class="participant-item">${ui.escape(p.nickname)}</div>`).join('');
}
...
function addChatLine(chat) {
    ...
    div.innerHTML = `<span style="color:${chat.profile.color || '#00ff94'}">${ui.escape(chat.profile.nickname)}</span>: ${ui.escape(chat.message)}`;
    box.appendChild(div);
...
// ============================================
// Professional Actions (Complete Functionality)
// ============================================
window.addCommand = () => {
...
window.editCommand = (trigger) => {
    ...
    if (cmd) {
        document.getElementById('new-command-trigger').value = cmd.triggers ? cmd.triggers[0] : cmd.trigger;
        document.getElementById('new-command-response').value = cmd.response;
        ui.modal('add-command-modal', true);
    }
};
...
window.editMacro = (id) => {
    ...
    if (m) {
        document.getElementById('new-macro-interval').value = m.interval;
        document.getElementById('new-macro-message').value = m.message;
        ui.modal('add-macro-modal', true);
    }
};
...
window.copyOverlayUrl = () => {
    ...
    ui.notify('URL이 복사되었습니다!', 'success');
};
