...
import ToastContainer from '@/components/ui/Toast'; // 신규 알림 시스템
        ...
        <AnimatePresence>
          {winner && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-8 pointer-events-none">
              <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} className="w-full max-w-2xl bg-black/90 backdrop-blur-3xl border border-emerald-500/30 rounded-[4rem] p-12 shadow-[0_0_100px_rgba(16,185,129,0.2)] pointer-events-auto text-center relative">
                ...
                <h3 className="text-sm font-black text-emerald-500 uppercase tracking-[0.5em] mb-4 animate-pulse">Winner</h3>
                ...
                <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-8 text-left h-40 overflow-y-auto space-y-4 pr-4 custom-scrollbar">
                  {winnerChats.length === 0 ? <p className="text-gray-600 font-bold italic text-center py-10">채팅 대기 중...</p> : winnerChats.map((chat, i) => (
