      ...
      <Modal isOpen={isBallotModalOpen} onClose={() => setIsBallotModalOpen(false)} title="투표자 상세 현황">
          <div className="space-y-6">
              ...
              <div className="max-h-[500px] overflow-y-auto custom-scrollbar space-y-6 pr-2">
                  {(currentVote && currentVote.options) || (history.find(h => h.id === targetVoteId)?.options) ? (currentVote?.options || history.find(h => h.id === targetVoteId)?.options).map((opt: any) => {
                      ...
                      return (
                          <div key={opt.id} className="bg-white/5 rounded-2xl p-4 border border-white/5">
                              ...
                              <div className="flex flex-wrap gap-2">{voters.map((b, idx) => (<div key={idx} className="inline-flex items-center gap-2 bg-black/20 px-3 py-2 rounded-lg border border-white/5 hover:border-emerald-500/30 transition-all"><div className="w-5 h-5 rounded-full bg-white/10 flex items-center justify-center text-[9px] font-black text-gray-400">{idx+1}</div><span className="text-sm font-medium text-gray-300">{showNicknames ? b.nickname : `익명(${b.userIdHash.substring(0,4)})`}</span></div>))}</div>
                          </div>
                      );
                  }) : (
                      <div className="flex flex-wrap gap-2">{ballots.map((b, idx) => (<div key={idx} className="inline-flex items-center gap-2 bg-black/20 px-3 py-2 rounded-lg border border-white/5"><span className="text-sm font-medium text-gray-300">{showNicknames ? b.nickname : `익명(${b.userIdHash.substring(0,4)})`}</span></div>))}</div>
                  )}
              </div>
          </div>
      </Modal>
