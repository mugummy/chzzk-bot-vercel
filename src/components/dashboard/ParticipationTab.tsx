...
export default function ParticipationTab({ onSend }: { onSend: (msg: any) => void }) {
  ...
  const handleSaveSettings = () => {
    ...
    if (typeof window !== 'undefined' && (window as any).ui?.notify) {
      (window as any).ui.notify('참여 설정이 적용되었습니다.', 'success');
    }
  };
      ...
      <div className="bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-12 flex flex-col xl:flex-row justify-between items-center gap-10 shadow-2xl">
        <div className="flex items-center gap-10 w-full xl:w-auto">
          <div className="space-y-3 flex-1 xl:w-64">
            ...
            <input value={cmd} onChange={e => setCmd(e.target.value)} className="w-full bg-white/5 border border-white/10 p-5 rounded-2xl focus:border-emerald-500 outline-none font-black text-xl text-white" />
          </div>
          ...
          <button onClick={handleSaveSettings} className="bg-white text-black p-5 rounded-2xl hover:bg-emerald-500 transition-all mt-6 shadow-xl"><Save size={24}/></button>
        </div>

        <div className="flex gap-4 w-full xl:w-auto">
          <button onClick={handleToggle} className={`flex-1 xl:px-12 py-6 rounded-[2rem] font-black flex items-center justify-center gap-3 transition-all hover:scale-105 ${participation.isActive ? 'bg-red-500 text-white shadow-red-500/20' : 'bg-emerald-500 text-black shadow-emerald-500/20 shadow-xl'}`}>
      ...
      <div className="grid grid-cols-12 gap-8">
            ...
            <div className="flex-1 space-y-3 overflow-y-auto pr-2 custom-scrollbar">
              {participation.queue.map((p, i) => (
                <motion.div initial={{opacity:0, x:-20}} animate={{opacity:1, x:0}} key={p.userIdHash} className="bg-white/[0.03] p-5 rounded-2xl border border-white/5 flex items-center justify-between group hover:border-cyan-500/30 transition-all">
                  ...
                  <button onClick={() => handleApprove(p.userIdHash)} className="px-6 py-2.5 bg-cyan-500 text-black font-black rounded-xl hover:scale-105 transition-all">승인</button>
                </motion.div>
              ))}
            </div>
          </div>
        ...
        <div className="col-span-12 xl:col-span-4 bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-10 flex flex-col shadow-2xl">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h4 className="text-xl font-black italic uppercase tracking-tighter text-white">참여왕 명예의 전당</h4>
          </div>
