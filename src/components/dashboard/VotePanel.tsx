...
export default function VotePanel({ onSend }: { onSend: (msg: any) => void }) {
  ...
  const [overlayConfig, setOverlayConfig] = useState(settings?.overlay || {
    backgroundColor: '#000000', textColor: '#ffffff', accentColor: '#10b981', opacity: 0.9, scale: 1.0
  });
  ...
  const handleResetVote = () => { onSend({ type: 'resetVote' }); setVoteQuestion(''); setVoteOptions([{id:'1',text:''},{id:'2',text:''}]); notify('초기화됨'); };
  ...
  const handleRemoveRouletteItem = (index: number) => { if (rouletteItems.length <= 1) return; setRouletteItems(rouletteItems.filter((_, i) => i !== index)); };
  const handleCreateRoulette = () => {
    if (rouletteItems.some(i => !i.text)) return notify('내용을 입력해주세요', 'error');
    onSend({ type: 'createRoulette', payload: { items: rouletteItems } });
    notify('룰렛 생성 요청');
  };
  const handleResetRoulette = () => { onSend({ type: 'resetRoulette' }); setRouletteItems([{id:'1',text:'',weight:1,color:'#10b981'}]); notify('초기화됨'); };
  const handleSpinRoulette = () => { onSend({ type: 'spinRoulette' }); notify('룰렛 회전 시작'); };

  const handleToggleDraw = (mode: 'chat' | 'donation') => {
    if (draw.isActive) { onSend({ type: 'stopDraw' }); notify('모집 마감 요청'); } 
    else { onSend({ type: 'startDraw', payload: { settings: { mode, chatType, chatCommand, donationType, donationAmount } } }); notify('모집 시작 요청'); }
  };
  ...
  const handleResetDraw = () => { onSend({ type: 'resetDraw' }); notify('초기화됨'); };
          ...
          <div className="xl:col-span-5 space-y-8">
            <div className="bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-10 shadow-2xl space-y-8">
              ...
              <div className="space-y-4">
                <input value={voteQuestion} onChange={e => setVoteQuestion(e.target.value)} className="w-full bg-white/5 border border-white/10 p-5 rounded-2xl outline-none font-bold text-lg text-white focus:border-emerald-500/50" placeholder="질문 입력" />
      ...
      {activeSubTab === 'roulette' && (
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
          ...
          <div className="xl:col-span-7 bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-10 shadow-2xl flex flex-col items-center justify-center min-h-[450px]">
            ...
            ) : <div className="opacity-20 flex flex-col items-center gap-6"><PieChart size={80} /><p className="text-2xl font-black italic">룰렛 항목을 생성해주세요.</p></div>}
          </div>
        </div>
      )}
      ...
      {['viewerDraw', 'donationDraw'].includes(activeSubTab) && (
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
          <div className="xl:col-span-5 bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-10 shadow-2xl space-y-8">
            ...
            <button onClick={onSend.bind(null, { type: 'resetDraw' })} className="w-full py-4 bg-white/5 text-gray-500 rounded-2xl font-bold text-xs hover:bg-white/10 transition-all">초기화</button>
          </div>
          <div className="xl:col-span-7 bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-10 shadow-2xl flex flex-col items-center justify-center min-h-[450px]">
            {draw.isActive ? (
              <div className="text-center space-y-6 w-full"><div className={`text-6xl font-black animate-pulse ${activeSubTab === 'viewerDraw' ? 'text-cyan-500 shadow-cyan-500/20' : 'text-amber-500 shadow-amber-500/20'}`}>{draw.candidatesCount}명</div><p className="text-white font-bold text-xl uppercase tracking-widest">현재 참여 중...</p><div className="grid grid-cols-3 gap-2 w-full p-4 bg-black/20 rounded-3xl max-h-[300px] overflow-y-auto custom-scrollbar">{draw.candidates.map((c, i) => <span key={i} className="text-xs bg-white/10 px-3 py-2 rounded-lg text-gray-300 truncate">{maskName(c.nickname)}</span>)}</div></div>
              ...
              <div className="w-full space-y-4">{draw.winners.map((w, i) => <motion.div initial={{y:20,opacity:0}} animate={{y:0,opacity:1}} transition={{delay:i*0.1}} key={i} className={`${activeSubTab === 'viewerDraw' ? 'bg-cyan-500' : 'bg-amber-500'} text-black p-6 rounded-3xl font-black text-3xl flex justify-between items-center shadow-xl`}><span>{w.nickname}</span><Trophy size={28} /></motion.div>)}</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="bg-white/[0.02] p-8 rounded-[3rem] border border-white/5 space-y-6">
              ...
              <div className="flex gap-3">
                <div className="flex-1 relative group"><input type={showUrl ? "text" : "password"} value={overlayUrl} readOnly className="flex-1 bg-black/40 border border-white/10 p-5 rounded-2xl text-emerald-400 font-mono text-xs outline-none" /><button onClick={() => setShowUrl(!showUrl)} className="p-2 text-gray-500">{showUrl ? <EyeOff/> : <Eye/>}</button></div>
                <button onClick={() => { navigator.clipboard.writeText(overlayUrl); notify('주소가 복사되었습니다.'); }} className="bg-emerald-500 text-black px-8 rounded-2xl font-black hover:bg-emerald-400 transition-all flex items-center gap-2 shadow-lg"><Copy size={18}/> 복사</button>
              </div>
            </div>
            <div className="bg-white/[0.02] p-8 rounded-[3rem] border border-white/5 space-y-6">
              <h4 className="text-xl font-black text-white flex items-center gap-3"><Palette className="text-emerald-500" /> 디자인 설정</h4>
              <div className="space-y-6">
                <div className="bg-white/5 p-5 rounded-[2rem] flex justify-between items-center border border-white/5 hover:border-emerald-500/30 transition-all"><div className="flex items-center gap-3"><Palette size={20} className="text-emerald-500"/><span className="font-bold text-gray-300">포인트 컬러</span></div><div className="neon-color-wrapper"><input type="color" value={overlayConfig.accentColor} onChange={e => setOverlayConfig({...overlayConfig, accentColor: e.target.value})} className="neon-color-picker" /></div></div>
                <div className="bg-white/5 p-5 rounded-[2rem] flex flex-col gap-3 border border-white/5"><div className="flex justify-between"><span className="font-bold text-gray-300 flex items-center gap-2"><Eye size={18} className="text-emerald-500"/> 배경 투명도</span><span className="font-black text-emerald-500">{Math.round(overlayConfig.opacity * 100)}%</span></div><input type="range" min="0" max="1" step="0.1" value={overlayConfig.opacity} onChange={e => setOverlayConfig({...overlayConfig, opacity: parseFloat(e.target.value)})} className="neon-slider w-full" /></div>
                <button onClick={handleSaveOverlay} className="w-full py-5 bg-emerald-500 text-black rounded-[2rem] font-black hover:scale-105 transition-all shadow-xl shadow-emerald-500/20 flex items-center justify-center gap-2 active:scale-95"><Save size={20}/> 설정 저장하기</button>
              </div>
            </div>
          </div>
        </div>
