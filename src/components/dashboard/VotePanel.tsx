...
export default function VotePanel({ onSend }: { onSend: (msg: any) => void }) {
  ...
  // 디버깅: 데이터 수신 확인
  useEffect(() => {
    console.log('[Dashboard] Votes Updated:', votes);
    console.log('[Dashboard] Roulette Updated:', roulette);
    console.log('[Dashboard] Draw Updated:', draw);
  }, [votes, roulette, draw]);
  ...
  const handleCreateVote = () => { if (!voteQuestion || voteOptions.some(o => !o.text)) return notify('빈 칸을 채워주세요', 'error'); onSend({ type: 'createVote', data: { question: voteQuestion, options: voteOptions } }); notify('투표 생성 요청 보냄'); };
  const handleResetVote = () => { onSend({ type: 'resetVote' }); setVoteQuestion(''); setVoteOptions([{id:'1',text:''},{id:'2',text:''}]); notify('투표 초기화 요청 보냄'); };
  const handleEndVote = () => { onSend({ type: 'endVote' }); notify('투표 마감 요청 보냄'); };
  ...
  const handleCreateRoulette = () => { if (rouletteItems.some(i => !i.text)) return notify('내용을 입력해주세요', 'error'); onSend({ type: 'createRoulette', payload: { items: rouletteItems } }); notify('룰렛 생성 요청 보냄'); };
  const handleResetRoulette = () => { onSend({ type: 'resetRoulette' }); setRouletteItems([{id:'1',text:'',weight:1,color:'#10b981'}]); notify('룰렛 초기화 요청 보냄'); };
  const handleSpinRoulette = () => { onSend({ type: 'spinRoulette' }); notify('룰렛 회전 요청 보냄'); };

  const handleToggleDraw = (mode: 'chat' | 'donation') => { if (draw.isActive) { onSend({ type: 'stopDraw' }); notify('마감 요청 보냄'); } else { onSend({ type: 'startDraw', payload: { settings: { mode, chatType, chatCommand, donationType, donationAmount } } }); notify('시작 요청 보냄'); } };
  const handleExecuteDraw = (fromVote: boolean = false, voteId?: string) => { onSend({ type: 'executeDraw', payload: { count: drawCount, fromVote, voteId } }); notify('추첨 실행 요청 보냄'); };
  const handleResetDraw = () => { onSend({ type: 'resetDraw' }); notify('추첨 초기화 요청 보냄'); };
  ...
  return (
    <div className="space-y-8">
      <header className="flex gap-3 p-2 bg-white/5 rounded-[2.5rem] border border-white/5 w-fit overflow-x-auto custom-scrollbar">
        {[{ id: 'vote', icon: <BarChart3 size={18}/>, label: '실시간 투표' }, { id: 'roulette', icon: <PieChart size={18}/>, label: '행운의 룰렛' }, { id: 'viewerDraw', icon: <Users size={18}/>, label: '시청자 추첨' }, { id: 'donationDraw', icon: <Coins size={18}/>, label: '후원 추첨' }, { id: 'settings', icon: <Settings2 size={18}/>, label: '오버레이 설정' }].map(tab => (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
          <div className="xl:col-span-5 bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-10 shadow-2xl space-y-8">
            ...
            <div className="space-y-6">
              ...
              {activeSubTab === 'donationDraw' && donationType === 'specific' && <div className="space-y-2"><label className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">설정 금액</label><NumberInput value={donationAmount} onChange={setDonationAmount} step={100} min={100} unit="치즈" /></div>}
          ...
          <div className="xl:col-span-7 bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-10 shadow-2xl flex flex-col items-center justify-center min-h-[450px]">
            {draw.isActive ? (
              <div className="text-center space-y-6 w-full"><div className={`text-6xl font-black animate-pulse ${activeSubTab === 'viewerDraw' ? 'text-cyan-500' : 'text-amber-500'}`}>{draw.candidatesCount}명</div><p className="text-white font-bold text-xl uppercase tracking-widest">현재 참여 중...</p><div className="grid grid-cols-3 gap-2 w-full p-4 bg-black/20 rounded-3xl max-h-[300px] overflow-y-auto custom-scrollbar">{draw.candidates.map((c, i) => <span key={i} className="text-xs bg-white/10 px-3 py-2 rounded-lg text-gray-300 truncate">{maskName(c.nickname)}</span>)}</div></div>
        ...
        <div className="bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-12 shadow-2xl space-y-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white/[0.02] p-8 rounded-[3rem] border border-white/5 space-y-6">
              ...
              <div className="space-y-4"><div className="flex justify-between items-center bg-white/5 p-4 rounded-2xl"><span className="text-sm font-bold text-gray-400">포인트 컬러</span><input type="color" value={overlayConfig.accentColor} onChange={e => setOverlayConfig({...overlayConfig, accentColor: e.target.value})} className="bg-transparent border-none w-8 h-8 cursor-pointer" /></div><div className="flex justify-between items-center bg-white/5 p-4 rounded-2xl"><span className="text-sm font-bold text-gray-400">투명도</span><input type="range" min="0" max="1" step="0.1" value={overlayConfig.opacity} onChange={e => setOverlayConfig({...overlayConfig, opacity: parseFloat(e.target.value)})} /></div><button onClick={handleSaveOverlay} className="w-full py-4 bg-emerald-500 text-black rounded-2xl font-black"><Save/> 저장</button></div>
            </div>
          </div>
        </div>
      )}
