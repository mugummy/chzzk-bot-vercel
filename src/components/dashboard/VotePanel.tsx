...
export default function VotePanel({ onSend }: { onSend: (msg: any) => void }) {
  ...
  const handleCreateRoulette = () => {
    if (rouletteItems.some(i => !i.text)) return notify('룰렛 항목을 모두 입력해주세요.', 'error');
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
          <div className="xl:col-span-5 bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-10 shadow-2xl space-y-8">
            ...
            <div className="space-y-4">
              <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">질문</label>
              <input value={voteQuestion} onChange={e => setVoteQuestion(e.target.value)} className="w-full bg-white/5 border border-white/10 p-5 rounded-2xl outline-none font-bold text-lg text-white focus:border-emerald-500/50" placeholder="예: 오늘 방송 어땠나요?" />
              <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1 block mt-4">항목</label>
            ...
            <div className="grid grid-cols-2 gap-4 pt-4">
              ...
              <button onClick={() => { onSend({ type: 'resetVote' }); setVoteQuestion(''); setVoteOptions([{id:'1',text:''},{id:'2',text:''}]); notify('투표가 초기화되었습니다.', 'info'); }} className="bg-white/5 text-gray-400 py-5 rounded-2xl font-black hover:bg-white/10 transition-all">초기화</button>
            </div>
          </div>
              ...
              {currentVote ? (
                <div className="w-full space-y-8 relative z-10">
                  ...
                  <div className="flex justify-center gap-4 mt-8">
                    ...
                    <button onClick={() => { onSend({ type: 'executeDraw', payload: { count: 1, fromVote: true } }); notify('투표 참여자 중에서 추첨합니다!'); }} className="px-10 py-4 bg-amber-500 text-black rounded-2xl font-black hover:scale-105 transition-all flex items-center gap-2 shadow-xl shadow-amber-500/20"><Trophy size={20}/> 투표자 추첨</button>
                  </div>
                </div>
              ) : (
                ...
                <header className="flex justify-between items-center mb-8">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2 bg-white/5 px-4 py-2 rounded-xl border border-white/5">
                      <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Reveal Names</span>
                    ...
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {currentVote.voters?.map((v: any, i: number) => (
                        <div key={i} className="bg-white/5 p-4 rounded-xl border border-white/5 flex flex-col">
                          ...
                          <span className="text-[9px] text-gray-500 font-bold uppercase mt-1">Choice: #{v.optionId.slice(-1)}</span>
                        </div>
                      ))}
                    </motion.div>
                  )}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
          <div className="xl:col-span-5 bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-10 shadow-2xl space-y-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <button onClick={() => { onSend({ type: 'resetRoulette' }); setRouletteItems([{id:'1',text:'',weight:1,color:'#10b981'}]); notify('초기화되었습니다.', 'info'); }} className="bg-white/5 text-gray-400 py-5 rounded-2xl font-black hover:bg-white/10 transition-all">초기화</button>
            </div>
            <button onClick={() => onSend({ type: 'spinRoulette' })} disabled={roulette.isSpinning} className="w-full bg-emerald-500 text-black py-5 rounded-2xl font-black hover:scale-105 transition-all shadow-xl flex items-center justify-center gap-3">
              <RotateCw size={20} className={roulette.isSpinning ? 'animate-spin' : ''} /> <span>룰렛 돌리기</span>
            </button>
          </div>
          <div className="xl:col-span-7 bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-10 shadow-2xl flex flex-col items-center justify-center min-h-[450px]">
            {roulette?.items?.length > 0 ? (
              <div className="text-center space-y-10">
                ...
                <AnimatePresence>
                  {roulette.winner && (
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="bg-emerald-500/20 text-emerald-400 px-10 py-5 rounded-3xl font-black text-3xl border border-emerald-500/30">🎉 {roulette.winner.text}</motion.div>
                  )}
                </AnimatePresence>
              </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
          <div className="xl:col-span-5 bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-10 shadow-2xl space-y-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <button onClick={() => { onSend({ type: 'executeDraw', payload: { count: drawCount } }); notify('시청자 추첨을 진행합니다!'); }} disabled={draw.isRolling} className="bg-cyan-500 text-black py-5 rounded-2xl font-black hover:scale-105 shadow-xl flex items-center justify-center gap-2"><Trophy size={20}/> 추첨하기</button>
            </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
          <div className="xl:col-span-5 bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-10 shadow-2xl space-y-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <button onClick={() => { onSend({ type: 'executeDraw', payload: { count: drawCount } }); notify('후원자 중 추첨을 진행합니다!'); }} disabled={draw.isRolling} className="bg-amber-500 text-black py-5 rounded-2xl font-black hover:scale-105 shadow-xl flex items-center justify-center gap-2"><Trophy size={20}/> 추첨하기</button>
            </div>
            <button onClick={() => onSend({ type: 'resetDraw' })} className="w-full py-4 bg-white/5 text-gray-500 rounded-2xl font-bold text-xs hover:bg-white/10 transition-all">초기화</button>
          </div>
          <div className="xl:col-span-7 bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-10 shadow-2xl flex flex-col items-center justify-center min-h-[450px]">
        ...
        <div className="bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-12 shadow-2xl space-y-10">
          ...
          <div className="bg-white/[0.02] p-10 rounded-[3rem] border border-white/5 space-y-6">
            ...
            <div className="flex gap-4 p-6 bg-white/5 rounded-2xl border border-white/5 items-start">
              ...
              <p className="text-xs text-gray-500 leading-relaxed font-medium">이 주소에는 사용자 세션 정보가 포함되어 있습니다. 타인에게 노출되지 않도록 주의하세요. 투표, 룰렛, 추첨 화면이 이 하나의 링크에서 자동으로 전환되어 표시됩니다.</p>
            </div>
          </div>
        </div>
      )}
