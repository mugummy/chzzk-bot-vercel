...
import { Vote, Plus, Trash2, Play, StopCircle, RotateCcw, Users, Activity, Gift, Dices, Settings2, Link, Monitor, ShieldCheck, CheckSquare, Square, ChevronRight } from 'lucide-react';
      ...
      <div className="flex gap-2 p-2 bg-white/5 rounded-3xl w-fit border border-white/5 shadow-2xl">
        {[
          ...
          <button
            ...
            className={`flex items-center gap-3 px-8 py-4 rounded-2xl font-black transition-all ${subTab === tab.id ? 'bg-emerald-500 text-black shadow-lg shadow-emerald-500/20 shadow-xl' : 'text-gray-500 hover:text-white hover:bg-white/5'}`}
          >
            <tab.icon size={18} />
      ...
      <AnimatePresence mode="wait">
              ...
              {!currentVote ? <div className="h-full flex flex-col items-center justify-center text-gray-700 py-32"><Vote size={80} className="opacity-20 mb-4"/><p className="font-bold italic tracking-tight">진행 중인 투표가 없습니다.</p></div> : (
                <div className="space-y-10">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="bg-emerald-500/20 text-emerald-400 px-4 py-1.5 rounded-xl text-[10px] font-black animate-pulse uppercase tracking-widest">Live Monitor</div>
                  </div>
        ...
        {subTab === 'draw' && (
          <motion.div key="draw" initial={{opacity:0, scale:0.95}} animate={{opacity:1, scale:1}} className="bg-[#0a0a0a] border border-white/5 p-16 rounded-[4rem] flex flex-col items-center text-center space-y-12 shadow-2xl relative overflow-hidden">
            ...
            <h2 className="text-5xl font-black tracking-tighter text-white uppercase">Draw System</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-3xl">
              <input value={drawKeyword} onChange={e => setDrawKeyword(e.target.value)} className="w-full bg-white/5 border border-white/10 p-6 rounded-[2rem] focus:border-emerald-500 outline-none font-black text-3xl text-center text-white" placeholder="!참여" />
              <input type="number" value={winnerCount} onChange={e => setWinnerCount(parseInt(e.target.value))} className="w-full bg-white/5 border border-white/10 p-6 rounded-[2rem] focus:border-emerald-500 outline-none font-black text-3xl text-center text-white" />
            </div>
        ...
        {subTab === 'settings' && (
          <motion.div key="settings" initial={{opacity:0}} animate={{opacity:1}} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="bg-[#0a0a0a] border border-white/5 p-12 rounded-[3.5rem] space-y-10 text-white shadow-2xl">
              <h3 className="text-2xl font-black flex items-center gap-2 uppercase tracking-tighter"><Link size={24} className="text-emerald-500"/> Browser Sources</h3>
              <div className="space-y-6">
                <div className="space-y-2">
                  <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Live Vote Overlay</p>
                  <button onClick={() => copyUrl('vote')} className="w-full py-5 bg-emerald-500 text-black font-black rounded-2xl hover:bg-emerald-400 transition-all shadow-xl shadow-emerald-500/20">URL 복사하기</button>
                </div>
                <div className="space-y-2">
                  <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Visual Song Player</p>
                  <button onClick={() => copyUrl('player')} className="w-full py-5 bg-white/5 text-white border border-white/10 font-black rounded-2xl hover:bg-white/10 transition-all">URL 복사하기</button>
                </div>
              </div>
            </div>
