...
import { 
  ...
  Coins, Copy, Eye, EyeOff, Dices, UserCheck, ShieldCheck, Palette, Save, Clock, ChevronRight
} from 'lucide-react';
...
export default function VotePanel({ onSend }: { onSend: (msg: any) => void }) {
  ...
  const handleEndVote = () => { onSend({ type: 'endVote' }); notify('투표가 마감되었습니다.'); };
  ...
  const handleSpinRoulette = () => { onSend({ type: 'spinRoulette' }); notify('룰렛을 돌립니다!'); };
          ...
          <div className="xl:col-span-5 space-y-8">
            <div className="bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-10 shadow-2xl space-y-8">
              ...
              <div className="space-y-4">
                <input value={voteQuestion} onChange={e => setVoteQuestion(e.target.value)} className="w-full bg-white/5 border border-white/10 p-5 rounded-2xl outline-none font-bold text-lg text-white" placeholder="질문 입력" />
                <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                  {voteOptions.map((opt, i) => (
                    <div key={opt.id} className="flex gap-3"><input value={opt.text} onChange={e => { const n = [...voteOptions]; n[i].text = e.target.value; setVoteOptions(n); }} className="flex-1 bg-white/5 border border-white/10 p-4 rounded-xl outline-none text-white font-medium" placeholder={`옵션 ${i + 1}`} /><button onClick={() => { if(voteOptions.length > 2) setVoteOptions(voteOptions.filter((_, idx) => idx !== i)); }} className="p-4 bg-red-500/10 text-red-500 rounded-xl hover:bg-red-500 transition-all"><Trash2 size={20}/></button></div>
                  ))}
                </div>
            ...
            <div className="bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-10 shadow-2xl flex flex-col items-center justify-center text-center min-h-[450px]">
              {currentVote ? (
                <div className="w-full space-y-8">
                  ...
                  <div className="space-y-4 text-left">{currentVote.options.map((opt, i) => { const percent = Math.round((currentVote.results[opt.id] / (currentVote.totalVotes || 1)) * 100); return <div key={opt.id} className="bg-white/5 p-5 rounded-2xl border border-white/5 flex justify-between relative overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${percent}%` }} className="absolute left-0 top-0 bottom-0 bg-emerald-500/10" /><span className="relative z-10 font-bold text-lg"><span className="text-emerald-500 mr-3">{i+1}.</span>{opt.text}</span><span className="relative z-10 font-black text-2xl text-emerald-500">{currentVote.results[opt.id]}표</span></div> })}</div>
                  <div className="flex justify-center gap-4 mt-8"><button onClick={() => onSend({ type: 'startVote' })} disabled={currentVote.isActive} className="px-10 py-4 bg-emerald-500 text-black rounded-2xl font-black hover:scale-105 disabled:opacity-30 flex items-center gap-2"><Play size={20}/> 시작</button><button onClick={handleEndVote} disabled={!currentVote.isActive} className="px-10 py-4 bg-red-500 text-white rounded-2xl font-black hover:scale-105 disabled:opacity-30 flex items-center gap-2"><StopCircle size={20}/> 종료</button><button onClick={() => handleExecuteDraw(true)} className="px-10 py-4 bg-amber-500 text-black rounded-2xl font-black hover:scale-105 flex items-center gap-2 shadow-xl shadow-amber-500/20"><Trophy size={20}/> 투표자 추첨</button></div>
                </div>
            ...
            {roulette?.items?.length > 0 ? (
              <div className="text-center space-y-10 w-full flex flex-col items-center">
                <motion.div animate={{ rotate: roulette.isSpinning ? 3600 : 0 }} transition={{ duration: 3, ease: "circOut" }} className="w-72 h-72 rounded-full border-[12px] border-white/5 flex items-center justify-center relative shadow-[0_0_50px_rgba(236,72,153,0.1)] bg-black overflow-hidden">
                  <div className="absolute inset-0 rounded-full overflow-hidden">{roulette.items.map((item, i) => <div key={i} className="absolute w-full h-full left-0 top-0 origin-center border-r border-white/5" style={{ transform: `rotate(${(360 / roulette.items.length) * i}deg)`, background: `conic-gradient(from 0deg, ${i%2===0?'#151515':'#0a0a0a'} 0deg ${(360/roulette.items.length)}deg, transparent ${(360/roulette.items.length)}deg)` }}></div>)}</div>
                  <PieChart size={120} className="text-pink-500 relative z-10" /><div className="absolute -top-6 left-1/2 -translate-x-1/2 w-0 h-0 border-l-[20px] border-l-transparent border-r-[20px] border-r-transparent border-t-[40px] border-t-white z-20 drop-shadow-xl" /></motion.div>
                ...
                <button onClick={handleSpinRoulette} disabled={roulette.isSpinning} className="w-full max-w-md bg-emerald-500 text-black py-5 rounded-2xl font-black hover:scale-105 transition-all shadow-xl flex items-center justify-center gap-3 disabled:opacity-50"><RotateCw size={24} className={roulette.isSpinning ? 'animate-spin' : ''} /> <span>룰렛 돌리기</span></button>
              </div>
        ...
        <div className="bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-12 shadow-2xl space-y-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="bg-white/[0.02] p-8 rounded-[3rem] border border-white/5 space-y-6">
              ...
              <div className="flex gap-3"><input type={showUrl ? "text" : "password"} value={overlayUrl} readOnly className="flex-1 bg-black/40 border border-white/10 p-5 rounded-2xl text-emerald-400 font-mono text-xs outline-none" /><button onClick={() => setShowUrl(!showUrl)} className="p-2 text-gray-500">{showUrl ? <EyeOff/> : <Eye/>}</button><button onClick={() => { navigator.clipboard.writeText(overlayUrl); notify('주소가 복사되었습니다.'); }} className="bg-emerald-500 text-black px-6 rounded-2xl font-black hover:bg-emerald-400 transition-all"><Copy/></button></div>
            </div>
            <div className="bg-white/[0.02] p-8 rounded-[3rem] border border-white/5 space-y-6">
              <h4 className="text-xl font-black text-white">디자인 설정</h4>
              <div className="space-y-6">
                <div className="bg-white/5 p-5 rounded-[2rem] flex justify-between items-center border border-white/5 hover:border-emerald-500/30 transition-all"><div className="flex items-center gap-3"><Palette size={20} className="text-emerald-500"/><span className="font-bold text-gray-300">포인트 컬러</span></div><div className="relative w-10 h-10 rounded-full overflow-hidden ring-2 ring-white/20"><input type="color" value={overlayConfig.accentColor} onChange={e => setOverlayConfig({...overlayConfig, accentColor: e.target.value})} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[150%] h-[150%] cursor-pointer p-0 border-0" /></div></div>
                <div className="bg-white/5 p-5 rounded-[2rem] flex flex-col gap-3 border border-white/5"><div className="flex justify-between"><span className="font-bold text-gray-300 flex items-center gap-2"><Eye size={18} className="text-emerald-500"/> 배경 투명도</span><span className="font-black text-emerald-500">{Math.round(overlayConfig.opacity * 100)}%</span></div><input type="range" min="0" max="1" step="0.1" value={overlayConfig.opacity} onChange={e => setOverlayConfig({...overlayConfig, opacity: parseFloat(e.target.value)})} className="w-full accent-emerald-500 h-2 bg-white/10 rounded-lg appearance-none cursor-pointer" /></div>
                <button onClick={handleSaveOverlay} className="w-full py-5 bg-emerald-500 text-black rounded-[2rem] font-black hover:scale-105 transition-all shadow-xl shadow-emerald-500/20 flex items-center justify-center gap-2"><Save size={20}/> 설정 저장하기</button>
              </div>
            </div>
          </div>
        </div>
