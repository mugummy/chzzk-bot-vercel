...
import { Terminal, Plus, Settings, Trash2, Search, Zap, Calculator, Edit3 } from 'lucide-react';
...
export default function CommandTab({ onSend }: { onSend: (msg: any) => void }) {
  ...
  // 헬퍼: 복수형과 단수형 트리거를 모두 지원 (빌드 에러 방지)
  const getDisplayTrigger = (cmd: CommandItem) => {
    return cmd.triggers?.[0] || cmd.trigger || '명령어';
  };
      ...
      <header className="flex flex-col lg:flex-row justify-between items-center gap-6">
        <div className="relative group w-full lg:w-96">
          ...
          <input 
            ...
            className="w-full bg-white/5 border border-white/5 pl-14 pr-6 py-5 rounded-[2rem] outline-none focus:border-emerald-500/50 transition-all text-white"
            placeholder="명령어 검색..."
          />
        </div>
        <div className="flex gap-4">
          <button onClick={() => { setModalMode('add'); setIsModalOpen(true); }} className="bg-emerald-500 text-black px-10 py-5 rounded-[2rem] font-black hover:scale-105 transition-all flex items-center gap-2 shadow-xl shadow-emerald-500/20">
            <Zap size={20} fill="currentColor" /> 명령어 추가
          </button>
          <button onClick={() => { setModalMode('counter'); setIsModalOpen(true); }} className="bg-white/5 border border-white/10 px-10 py-5 rounded-[2rem] font-black hover:bg-white/10 transition-all flex items-center gap-2">
            <Calculator size={20} /> 카운터 추가
          </button>
        </div>
      </header>
          ...
          {commands.filter(c => getDisplayTrigger(c).includes(search)).map((cmd, i) => (
            <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} key={cmd.id || i} className="bg-[#0a0a0a] border border-white/5 p-10 rounded-[3.5rem] flex items-center justify-between group hover:border-emerald-500/30 transition-all">
              <div className="flex items-center gap-8">
                ...
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="px-3 py-1 bg-emerald-500/10 text-emerald-500 text-[10px] font-black rounded-full">Command</span>
                  </div>
              ...
              <div className="flex gap-3 opacity-0 group-hover:opacity-100 transition-all">
                ...
                <button onClick={() => onSend({ type: 'removeCommand', data: { trigger: getDisplayTrigger(cmd) } })} className="p-5 bg-red-500/10 text-red-500 rounded-2xl hover:bg-red-500 hover:text-white transition-all shadow-lg"><Trash2 size={22} /></button>
              </div>
            </motion.div>
          ))}
          {counters.filter(c => (c.trigger || '').includes(search)).map((cnt, i) => (
            <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} key={`cnt-${i}`} className="bg-[#0a0a0a] border border-cyan-500/10 p-10 rounded-[3.5rem] flex items-center justify-between group hover:border-cyan-500/30 transition-all">
              <div className="flex items-center gap-8">
                ...
                <div>
                  <div className="flex items-center gap-3 mb-2"><span className="text-white font-black text-2xl">{cnt.trigger}</span><span className="px-3 py-1 bg-cyan-500/10 text-cyan-500 text-[10px] font-black rounded-full uppercase">Counter</span></div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={modalMode === 'counter' ? '카운터 설정' : '명령어 설정'} onSave={handleSave}>
        {modalMode === 'counter' ? (
          <div className="space-y-8">
            <input value={cntData.trigger} onChange={e => setCntData({...cntData, trigger: e.target.value})} className="w-full bg-white/5 border border-white/10 p-6 rounded-3xl outline-none font-black text-2xl text-white shadow-inner" placeholder="!키워드" />
            <textarea value={cntData.response} onChange={e => setCntData({...cntData, response: e.target.value})} className="w-full bg-white/5 border border-white/10 p-6 rounded-3xl outline-none font-medium h-32 text-white shadow-inner" placeholder="응답 메시지 {count} 변수 사용 가능" />
          ...
          <div className="space-y-8">
            <input value={cmdData.trigger} onChange={e => setCmdData({...cmdData, trigger: e.target.value})} className="w-full bg-white/5 border border-white/10 p-6 rounded-3xl outline-none font-black text-2xl text-white shadow-inner" placeholder="!명령어 / !다중" />
            <textarea value={cmdData.response} onChange={e => setCmdData({...cmdData, response: e.target.value})} className="w-full bg-white/5 border border-white/10 p-6 rounded-3xl outline-none font-medium h-48 text-white shadow-inner" placeholder="봇의 응답 내용을 입력하세요..." />
          </div>
        )}
      </Modal>
    </div>
