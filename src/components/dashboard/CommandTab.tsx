...
export default function CommandTab({ onSend }: { onSend: (msg: any) => void }) {
  ...
  const handleOpenEdit = (cmd: any) => {
    setCmdData({ trigger: cmd.triggers?.[0] || cmd.trigger, response: cmd.response });
    setEditingId(cmd.triggers?.[0] || cmd.trigger);
    setModalMode('edit');
    setIsModalOpen(true);
  };
    ...
    <div className="space-y-12">
      <header className="flex flex-col lg:flex-row justify-between items-center gap-6">
        <div className="relative group w-full lg:w-96">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} className="w-full bg-white/5 border border-white/5 pl-14 pr-6 py-5 rounded-[2rem] outline-none focus:border-emerald-500/50 transition-all" placeholder="검색어 입력..." />
        </div>
        <div className="flex gap-4 w-full lg:w-auto">
          ...
          <button onClick={() => { setModalMode('counter'); setIsModalOpen(true); }} className="flex-1 lg:flex-none bg-white/5 border border-white/10 px-10 py-5 rounded-[2rem] font-black hover:bg-white/10 transition-all flex items-center justify-center gap-3"><Calculator size={20} /> 카운터 추가</button>
        </div>
      </header>

      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        <AnimatePresence>
          {commands.filter(c => (c.triggers?.[0] || c.trigger || '').includes(search)).map((cmd, i) => (
            <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} key={i} className="bg-[#0a0a0a] border border-white/5 p-10 rounded-[3.5rem] flex items-center justify-between group hover:border-emerald-500/30 transition-all">
              <div className="flex items-center gap-8">
                ...
                <div>
                  <div className="flex items-center gap-3 mb-2"><span className="text-white font-black text-2xl">!{cmd.triggers?.[0] || cmd.trigger}</span><span className="px-3 py-1 bg-emerald-500/10 text-emerald-500 text-[10px] font-black rounded-full uppercase">Command</span></div>
                  <p className="text-gray-500 font-medium line-clamp-1">{cmd.response}</p>
                </div>
              </div>
              <div className="flex gap-3 opacity-0 group-hover:opacity-100 transition-all">
                <button onClick={() => handleOpenEdit(cmd)} className="p-5 bg-white/5 rounded-2xl hover:bg-white/10"><Edit3 size={22} /></button>
                <button onClick={() => onSend({ type: 'removeCommand', data: { trigger: cmd.triggers?.[0] || cmd.trigger } })} className="p-5 bg-red-500/10 text-red-500 rounded-2xl hover:bg-red-500 hover:text-white transition-all"><Trash2 size={22} /></button>
              </div>
            </motion.div>
          ))}
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={modalMode === 'counter' ? '카운터 설정' : '명령어 설정'} onSave={handleSave}>
        {modalMode === 'counter' ? (
          <div className="space-y-8">
            <input value={cntData.trigger} onChange={e => setCntData({...cntData, trigger: e.target.value})} className="w-full bg-white/5 border border-white/10 p-6 rounded-3xl outline-none font-black text-2xl" placeholder="!키워드" />
            <textarea value={cntData.response} onChange={e => setCntData({...cntData, response: e.target.value})} className="w-full bg-white/5 border border-white/10 p-6 rounded-3xl outline-none font-medium h-32" placeholder="응답 메시지 {count} 변수 사용 가능" />
            <label className="flex items-center justify-between p-6 bg-white/5 rounded-3xl border border-white/5 cursor-pointer hover:bg-white/10 transition-all">
              <span className="font-black">하루 한 번만 실행</span>
          ...
          <div className="space-y-8">
            <input value={cmdData.trigger} onChange={e => setCmdData({...cmdData, trigger: e.target.value})} className="w-full bg-white/5 border border-white/10 p-6 rounded-3xl outline-none font-black text-2xl" placeholder="!명령어 / !다중" />
            <textarea value={cmdData.response} onChange={e => setCmdData({...cmdData, response: e.target.value})} className="w-full bg-white/5 border border-white/10 p-6 rounded-3xl outline-none font-medium h-48" placeholder="봇의 응답 내용을 입력하세요..." />
          </div>
        )}
      </Modal>
    </div>
